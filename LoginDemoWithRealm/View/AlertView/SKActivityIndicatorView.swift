...
//            activityIndicatorView!.barTintColor = backgroundViewColor
    ...
    fileprivate func configureActivityIndicatorSize() {
        ...
        {
            let attributes = [kCTFontAttributeName : statusLabel?.font]
            
            let options: NSStringDrawingOptions = [.usesFontLeading, .truncatesLastVisibleLine, .usesLineFragmentOrigin]
            
            rectLabel = (statusLabel?.text?.boundingRect(with: CGSize(width: 200, height: 300), options: options, attributes: attributes as [NSAttributedStringKey : Any] , context: nil))!
